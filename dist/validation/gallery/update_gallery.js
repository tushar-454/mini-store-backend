...
const updateGalleryValidationSchema = zod_1.z.object({
    src: zod_1.z.string({ message: 'src is required' }).url({ message: 'src must be a valid URL' }).optional(),
    width: zod_1.z.number({ message: 'width is required' }).positive().optional(),
    height: zod_1.z.number({ message: 'height is required' }).positive().optional(),
    label: zod_1.z.string({ message: 'label is required' }).optional(),
});
const updateGalleryValidation = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
